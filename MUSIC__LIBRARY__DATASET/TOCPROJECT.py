...
import random
import sqlite3
image1='mohammad-metri-1oKxSKSOowE-unsplash.jpg'
image2='namroud-gorguis-FZWivbri0Xk-unsplash.jpg'
image3='wes-hicks-MEL-jJnm7RQ-unsplash.jpg'
#pip install Pillow
class menu:
    ...
    def __init__(self):
        ...
        conn.execute('''create table if not exists SONG_info
        ...
        conn.commit()
        conn.execute('''create table if not exists SONG_issued
        ...
        l1=Button(self.a,text='SONG DATA',font='Papyrus 22 bold',fg='Yellow',bg='Black',width=19,padx=10,borderwidth=0,command=self.song).place(x=100,y=500)
        l2=Button(self.a,text='USER DATA',font='Papyrus 22 bold',fg='Yellow',bg='Black',width=19,padx=10,borderwidth=0,command=self.user).place(x=800,y=500)
        self.root.mainloop()
    ...
    def song(self):
        ...
        l1=Button(self.a,text='Add Songs',font='Papyrus 22 bold',fg='Orange',bg='Black',width=15,padx=10,command=self.addsong).place(x=12,y=100)
        l2=Button(self.a,text='Search Songs',font='Papyrus 22 bold',fg='Orange',bg='Black',width=15,padx=10,command=self.search).place(x=12,y=200)

        l4=Button(self.a,text='All Songs',font='Papyrus 22 bold',fg='Orange',bg='Black',width=15,padx=10,command=self.all).place(x=12,y=300)
    ...
    def addsong(self):
        ...
        l1=Label(self.f1,text='Song ID : ',font='Papyrus 12 bold',fg='Orange',bg='Black',pady=1).place(x=50,y=50)
        ...
        l3=Label(self.f1,text='Singer : ',font='Papyrus 12 bold',fg='orange',bg='Black',pady=1).place(x=50,y=150)
        e3=Entry(self.f1,width=45,bg='orange',fg='black',textvariable=self.asinger).place(x=150,y=150)
    ...
    def adddata(self):
        ...
        try:
            if (a and b and c and d  and f)=="":
                ...
                conn.execute("insert into SONG_info \
            ...
            messagebox.showinfo("Error","Song is already present.")
    ...
    def search(self):
        ...
        self.f1=Frame(self.a,height=500,width=650,bg='black')
        self.f1.place(x=500,y=100)
        l1=Label(self.f1,text='SONG ID/Title/Singer/Genre: ',font=('Papyrus 10 bold'),bd=2, fg='orange',bg='black').place(x=20,y=40)
    ...
    def serch1(self):
        k=self.sid.get()
        if k!="":
            self.list4=("SONG ID","TITLE","SINGER","GENRE","COPIES","LOCATION")
          ...
          #for pattern in patterns:
               ...
               #if re.search ( pattern, SONG_info ):
                   #print ( 'SONG ID, Genre , SINGER ' )


            c=conn.execute("select * from SONG_info where ID=? OR TITLE=? OR SINGER=? OR GENRE=?",(k.capitalize(),k.capitalize(),k.capitalize(),k.capitalize(),))
            a=c.fetchall()
            if len(a)!=0:
                for row in a:
                ...
                self.cm.config(values =('Add Copies', 'Delete Copies', 'Delete Song'))
    ...
    def delete2(self):
        conn=sqlite3.connect('test.db')
        cd=conn.execute("select * from SONG_issued where SONG_ID=?",(self.c1,))
        ab=cd.fetchall()
        if ab!=0:
            conn.execute("DELETE FROM SONG_info where ID=?",(self.c1,))
            conn.commit()
            messagebox.showinfo("Successful","Song Deleted sucessfully.")
            self.trees.delete(self.curItem)
        else:
            messagebox.showinfo("Error","Song is Issued.\nBook cannot be deleted.")
        conn.commit()
        conn.close()
    ...
    def copiesadd(self):
        ...
        if int(no)>=0:
            ...
            conn.execute("update SONG_info set COPIES=COPIES+? where ID=?",(no,self.c1,))
            conn.commit()
    ...
    def copiesdelete(self):
        ...
        if int(no1)>=0:
            if int(no1)<=int(self.c2):
                ...
                conn.execute("update SONG_info set COPIES=COPIES-? where ID=?",(no1,self.c1,))
                conn.commit()
                conn.close()
    ...
    def all(self):
        ...
        conn=sqlite3.connect('test.db')
        self.list3=("SONG ID","TITLE","SINGER","GENRE","COPIES","LOCATION")
        self.treess=self.create_tree(self.f1,self.list3)
        self.treess.place(x=25,y=50)
        c=conn.execute("select * from SONG_info")
        g=c.fetchall()
        if len(g)!=0:
            for row in g:
    ...
    def user(self):
        self.a.destroy()
        self.a=self.canvases(image2)
        l1=Button(self.a,text='Issue Song',font='Papyrus 22 bold',fg='Orange',bg='Black',width=15,padx=10,command=self.issue).place(x=12,y=100)
        l2=Button(self.a,text='Return Song',font='Papyrus 22 bold',fg='Orange',bg='Black',width=15,padx=10,command=self.returnn).place(x=12,y=200)
        l3=Button(self.a,text='User Activity',font='Papyrus 22 bold',fg='Orange',bg='Black',width=15,padx=10,command=self.activity).place(x=12,y=300)
    ...
    def issue(self):
        ...
        self.f1.place(x=500,y=100)
        l1=Label(self.f1,text='SONG ID : ',font='papyrus 15 bold',bg='black',fg='orange').place(x=50,y=100)
        ...
        b1=Button(self.f1,text='Issue',font='Papyrus 10 bold',fg='black',bg='orange',width=10,bd=3,command=self.issuedsong).place(x=200,y=250)

    def issuedsong(self):
        songid=self.aidd.get()
        userid=self.auser.get()
        conn=sqlite3.connect('test.db')
        cursor=conn.cursor()
        cursor.execute("select ID,COPIES from SONG_info where ID=?",(songid.capitalize(),))
        an=cursor.fetchall()
        if (songid and userid!=""):
            if an!=[]:
                for i in an:
                    if i[1]>0:
                        try:
                            conn.execute("insert into song_issued \
                            values (?,?,date('now'),date('now','+7 day'))",(songid.capitalize(),userid.capitalize(),))
                            conn.commit()
                            conn.execute("update SONG_info set COPIES=COPIES-1 where ID=?",(songid.capitalize(),))
                            conn.commit()
                            conn.close()
                            messagebox.showinfo("Updated","Song Issued sucessfully.")
                        except:
                            messagebox.showinfo("Error","Song is already issued by student.")

                    else:
                        messagebox.showinfo("Unavailable","Song unavailable.\nThere are 0 copies of the book.")
            else:
                messagebox.showinfo("Error","No such Song in Database.")
        else:
            messagebox.showinfo("Error","Fields cannot be blank.")

    def returnn(self):
        self.aidd=StringVar()
        self.auser=StringVar()

        self.f1=Frame(self.a,height=550,width=500,bg='black')
        self.f1.place(x=500,y=100)
        l1=Label(self.f1,text='Song ID : ',font='papyrus 15 bold',fg='orange', bg='black').place(x=50,y=100)
        e1=Entry(self.f1,width=25,bd=4,bg='orange',textvariable=self.aidd).place(x=180,y=100)
    ...
    def returnsong(self):
        ...
        fg=conn.execute("select ID from SONG_info where ID=?",(a.capitalize(),))
        fh=fg.fetchall()
        conn.commit()
        if fh!=None:
            c=conn.execute("select * from SONG_issued where SONG_ID=? and USER_ID=?",(a.capitalize(),b.capitalize(),))
            d=c.fetchall()
            conn.commit()
            if len(d)!=0:
                c.execute("DELETE FROM SONG_issued where SONG_ID=? and USER_ID=?",(a.capitalize(),b.capitalize(),));
                conn.commit()
                conn.execute("update SONG_info set COPIES=COPIES+1 where ID=?",(a.capitalize(),))
                conn.commit()

                messagebox.showinfo("Success","Song Returned sucessfully.")
            else:
                messagebox.showinfo("Error","Data not found.")
        else:
            messagebox.showinfo("Error","No such Song.\nPlease add the Song in database.")
        conn.commit()
        conn.close()
    ...
    def activity(self):
        ...
        self.list2=("SONG ID","USER ID","ISSUE DATE","RETURN DATE")
        self.trees=self.create_tree(self.f1,self.list2)
        self.trees.place(x=50,y=150)


        l1=Label(self.f1,text='Song/User ID : ',font='Papyrus 15 bold',fg='Orange',bg='black').place(x=50,y=30)
    ...
    def searchact(self):
        ...
        try:
            c=conn.execute("select * from book_issued where SONG_ID=? or USER_ID=?",(bid.capitalize(),bid.capitalize(),))
            d=c.fetchall()
            if len(d)!=0:
                for row in d:
                    self.trees.insert("",END,values=row)
    ...
    def searchall(self):
        ...
        conn=sqlite3.connect('test.db')
        try:
            c=conn.execute("select * from SONG_issued")
            d=c.fetchall()
            for row in d:
                self.trees.insert("",END,values=row)
...
def Database():
    ...
    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `login` (username, password) VALUES('duttaaadhar@gmail.com', 'Password')")
        conn.commit()
